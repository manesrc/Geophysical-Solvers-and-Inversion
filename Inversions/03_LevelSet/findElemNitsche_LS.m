function [list_Omega1,list_Omega2,list_cut_elem,list_edge_elem1,list_edge_elem2, LABpoints] = findElemNitsche_LS(tol,plot1,InfoMesh,InfoLAB,InfoProblem)
...
function plot_crossed_elements(list_cut_elem,list_edge_elem1,list_edge_elem2,LABpoints,InfoMesh,InfoProblem)
    ...
    contourf(x_grid1, max(X(:,2)) - y_grid1, data2plot, 'FaceAlpha', 0.5);
    ...
    yplotLAB = LABpoints(2,:);%(InfoProblem.L_ref/1000 + InfoLAB.LABy)/(InfoProblem.L_ref/1000);
    ...
    legend('','','','elements correctly crossed','elements ill cross in \Omega_1','elements ill cross in \Omega_2','FontSize',12,'Location','SouthOutside','AutoUpdate','off')
