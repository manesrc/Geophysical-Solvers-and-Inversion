function [datos_x,datos_y] = increase_data_LAB(InfoLAB,data_increase)
...
num_tramos = orig_length-1;

datos_x = zeros(orig_length + num_tramos*data_increase,1);
datos_y = zeros(orig_length + num_tramos*data_increase,1);

fin_index = 1;
for ii = 1:num_tramos
    datos_x_ii = [dataLABx(ii) dataLABx(ii+1)];
    datos_y_ii = [dataLABy(ii) dataLABy(ii+1)];
    points_added_x = linspace(datos_x_ii(1), datos_x_ii(2),data_increase+2);
    if datos_x_ii(1) == datos_x_ii(2)
        points_added_y  = linspace(datos_y_ii(1),datos_y_ii(2),data_increase+2);
    else
        points_added_y = interp1(datos_x_ii,datos_y_ii,points_added_x);
    end 
    ini_index = fin_index;
    fin_index = ini_index + (data_increase+1);
