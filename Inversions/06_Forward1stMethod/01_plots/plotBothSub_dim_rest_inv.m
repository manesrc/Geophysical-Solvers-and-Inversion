function plotBothSub_dim_rest_inv(temp1,temp2,InfoMesh,InfoLAB,InfoProblem,nodes1,nodes2,grad2plot_mean,xplotgrad,fig_num,varargin)
% unfold mesh
T = InfoMesh.T;
X = InfoMesh.X;
...
InfoLAB.maxDepth = max(X(:,2));
minLAB = min(InfoLAB.LABy)/InfoLAB.maxDepth;
maxLAB = max(InfoLAB.LABy)/InfoLAB.maxDepth;
elem2add_Omega1 = ceil((InfoMesh.fin_y-InfoMesh.ini_y) * (minLAB/( (InfoMesh.fin_y-InfoMesh.ini_y) /ne_y) ));
elem2add_Omega2 = ceil((InfoMesh.fin_y-InfoMesh.ini_y) * (maxLAB/( (InfoMesh.fin_y-InfoMesh.ini_y) /ne_y) )); 
...
for ii_plot = 1:2
    ...
    yu = unique(round(Xe2plot(:,2),6)); % write in depth: L_max - x & in km /1000
    ...
    if ii_plot == 1
        ...
        sf1 = subplot(2,1,1);
        sf2 = subplot(2,1,2);
        sf1.Position = [0.1300    0.2450    0.7750    0.700];
        ...
        %sf1.XTick = [];
        sf1.XTickLabel = [];
        ylabel(sf1,'Depth [m]','FontSize',fonti_size)
        %grad2plot2= [grad2plot; grad2plot(end)];
        %plot(sf2,(InfoProblem.L_ref/1000)*xplotgrad,grad2plot_mean','k-','LineWidth',1.0)
        plot(sf2,xplotgrad,1000*grad2plot_mean','b-','LineWidth',1.0)
        vect1 = 1000*grad2plot_mean;
        axis_info = [min(X(:,1)) max(X(:,1)) min(min(vect1)) max(max(vect1))];
        ...
        sf2.YLim = [0.0 0.8];
        sf2.YTick = 0.0:0.2:0.8;
        sf2.YTickLabel = {'0.0','','0.4','','0.8'};
        xlabel(sf2,'X [m]','FontSize',fonti_size)
        ylabel(sf2,'\nabla T [ºC/km]','FontSize',fonti_size)
        set(sf2,'FontSize',fonti_size)
    else
        yu = sort(yu,'descend'); % for depth 'ascend' for "distance to the surf" put 'descend'
        figure(fig_num)
        hold(sf1,'on')
        contours2 = [1300:20:1360 1400:100:1700]';
        contours1 = [20 200:300:1100 1300];
        %contours2 = [1100:100:1400 1500:20:1580 1600:100:1800]';
        [C2,h2] = contourf(sf1,xu,(InfoLAB.maxDepth-yu),T_r,contours2);     
        %contours1 = [293; 500; 800; 1100; 1300; 1500];
        [C1,h1] = contourf(sf1,xu1,(InfoLAB.maxDepth-yu1),T_r1,contours1);
        set(sf1,'YDir','reverse')
        iso_line_width = 1.2;
        limits = [20 1600];
        colormap(jet)
        caxis(sf1, limits);
        ...
        plot(sf1,Xinterf,InfoLAB.maxDepth-InfoLAB.LABy,'k-','LineWidth',iso_line_width);
        plot(sf1,Xinterf,InfoLAB.maxDepth-InfoLAB.LABy,'b-','LineWidth',2.0)
        if varargin{1}.reference == 1
            data_x = varargin{1}.data_x; 
            data_y = varargin{1}.data_y; 
            plot(sf1,data_x,data_y,'--','LineWidth',2,'Color',[0.4940 0.1840 0.5560]);
        end 
    end 
