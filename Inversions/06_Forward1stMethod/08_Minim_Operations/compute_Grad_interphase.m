function [G_matrix,matrixS] = compute_Grad_interphase(nOfPoints_Elem,DOF_prob,problem_int,figurePointsInterface,InfoProblem,InfoMesh,Physical_area,min_ratio,Temp)
...
if isempty(InfoMesh.list_cut) ~= 1
    ...
    for ii = 1:size(InfoMesh.list_cut,1)
        ...
        % chekc if lemen
        if Elem_phys_area(1) < min_ratio || Elem_phys_area(1) > (1-min_ratio)
...
% % borrar
% contracted_gradient_elements = zeros(nOfPointsFlux,4);
% Temp_elem1 = zeros(nOfPointsFlux,4);
% position_points123 = zeros(nOfPointsFlux,2);
% indices123 = 1:1:length(DOF_prob);
% a1 = 0;
...
for jj = 1:nelem2consider           % element by element
    ...
    for qq=1:nOfPoints_Elem
        ...
        % % borrar 
        % contracted_gradient_elements(matrixRow+qq,:) = Contracted_Gradient';
        % %error('there is a sorting problem? the DOF follows a ascending that is not real')
        % tram1 = indices123(logical(sum(DOF_prob == Te,2)));
        % 
        % %logical1 = logical(sum(DOF_prob == Te,2));
        % Temp_elem1(matrixRow+qq,:) = Temp(tram1);
        % if max(tram1) > a1
        %     a1 = max(tram1);
        %     % esto está bien 
        % end 
        % position_points123(matrixRow+qq,:) = N_LABpoints(qq,:)*Xe;
        % 
...
% fluxis_nd = zeros(nOfPointsFlux,1);
% for ii = 1:length(fluxis_nd)
%     sort1 = [1 2 4 3]; 
%     fluxis_nd(ii) = InfoProblem.k1*contracted_gradient_elements(ii,:)*Temp_elem1(ii,sort1)';
% end
% 
% figure(12345); clf; scatter(position_points123(:,1),fluxis_nd)
% hold on; scatter(matrixS*X(:,1),G_matrix*Temp)
