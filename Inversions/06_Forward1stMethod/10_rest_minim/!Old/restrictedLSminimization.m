function T_2_nd2 = restrictedLSminimization(vel_01,K_fem2,N2,G2_matrix,f_fem2,vn2,matG1,matG2,T_1_nd,DOF1,DOF2,DOF_Q,PosMat,InfoLAB,InfoProblem,InfoMesh)
% create v_0 and B_0
% New Mat 2
...
x_length_elem = (1/InfoMesh.nel_x); % may be InfoMesh.fin_x-InfoMesh.ini_x instead of 1
...
% construct B_0
D_mat2 = eye(length(DOF_Q));
D_mat = [D_mat2; zeros(length(DOF2)-size(D_mat2,1),size(D_mat2,2))];
B_0 = matG2 * (MAT2_new2(DOF2,DOF2)\D_mat);
% solve a minimization problem with restrictions:
perturbation = optimvar('pert',length(DOF_Q),'LowerBound',((0.3-0.5)/0.5)*g_mean,'UpperBound',((0.6-0.5)/0.5)*g_mean);
residual = B_0*perturbation - v_0;
obj = sum(residual.^2);
minim_cond = optimproblem('Objective',obj);
[sol,fval,exitflag,output] = solve(minim_cond);
perturbation_long = D_mat*sol.pert;
vect2_new = f_fem2(DOF2)  + vn2(DOF2) + g_mean_long + perturbation_long;
...
num_plot = 113; 
varargin.perturbation = sol.pert;%D_mat2*g_perturbation;
...
text(330,mean(fluxes123),['Perf. minim. = ',num2str(round((1 - norm_final_flux/norm_contflux_gmean),3))],'FontSize',fontSize55)
text(330,min(fluxes123)+mean(fluxes123)/6,['flux. cont. meas. = ',num2str(round((1 - norm_final_flux/norm_flux1),3))],'FontSize',fontSize55)
